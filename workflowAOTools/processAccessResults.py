...
# TODO: Clean up internal variable names to match naming convention of write out.
...
def output(final_results, thrshld_list, file_base, file_updt):

    #make the new file name derived from the input file name
    file_name_base = file_base.replace("1-", "2-")
    file_name_updt = file_updt.replace("1-", "")

    with open(f"{file_name_base}-{file_name_updt}.csv", 'w') as outfile:
...
if __name__ == "__main__":
    ...
    # Make the new file name derived from the input file name
    fileBase = str(sys.argv[2]).replace(".csv", "")
    print("Base file name: ", fileBase)

    fileUpdt = str(sys.argv[4]).replace(".csv", "")
    print("Update file name: ", fileUpdt)
    ...
    output(final_results, thrshldlist, fileBase, fileUpdt)
