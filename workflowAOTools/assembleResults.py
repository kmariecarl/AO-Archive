...
class AccessFile:
    ...
    def average_results(self):
        ...
        bash_command = "python " \
                      "/Users/kristincarlson/Dropbox/Programs/gitBus-Highway/workflowAOTools/averageTransitLocal.py" \
                      " -r Access_Results/ -o Access_Results_Avg/ -access jobs"
        run_command(bash_command)
    ...
    def compare_access(self, compare_to_folder, compare_to_file):
        ...
        bash_command = f"python " \
            ...
            f" -bs {base} -updt {updt} -access jobs"
        run_command(bash_command)
    ...
    def all_ww_results(self, processed, rac):
        ...
        bash_command = f"python /Users/kristincarlson/Dropbox/Programs/gitBus-Highway/workflowAOTools/workWeightAccess.py" \
            f" -access {processed} -rac {rac} -or_lab label -rac_lab GEOID10 -worker_lab racC000 -all"
        run_command(bash_command)

    def group_ww_results(self, processed, rac, group):
        print("Message: Computing worker-weighted statistics for grouped blocks")
        bash_command = f"python /Users/kristincarlson/Dropbox/Programs/gitBus-Highway/workflowAOTools/workWeightAccess.py" \
            f" -access {processed} -rac {rac} -or_lab label -rac_lab GEOID10 -worker_lab racC000 -group {group}" \
...
if __name__ == '__main__':
    ...
    bar = bar.Bar(message ='Processing', fill='@', suffix='%(percent)d%%', max=12)
    ...
    if args.HALFM_FLAG == "Y":
        halfm_blocks = input("Type full file path to half mile blocks file (default available): ") or \
                       "/Users/kristincarlson/Dropbox/AO_Projects/met_council_analyses/2_transit/scenarios/HALFMILE_BLOCKS_FUNDED_BASELINE.csv"
    else:
        halfm_blocks = None
        print("Message: No half mile accessibility results will be made")

    if args.WW_ALL_FLAG == "Y":
        rac_confirm = input("Please confirm that 2015 RAC data can be used Y/N: ")
        if rac_confirm == "Y":
            rac = "/Users/kristincarlson/Dropbox/DataSets/WAC_RAC/2015/Joined_RAC_FILE_20181003095127.csv"
        else:
            rac = None
            print("Message: Please update RAC data path to alternative year. Exiting program")
            quit()
    else:
        rac = None
        rac_confirm = None
    ...
    if args.WW_HALFM_FLAG == "Y" and rac_confirm == "Y":
        group = input("Type path to file that maps block ids to group level id (default available): ") or\
                "/Users/kristincarlson/Dropbox/AO_Projects/met_council_analyses/2_transit/scenarios/GEOID10_GNISID.csv"
    else:
        group = None
    ...
    if args.CTU_TABLES_FLAG == "Y" and rac_confirm == "Y":
        ctu = input("Type path to blocks to CTU id file (default available): ") or "/Users/kristincarlson/Dropbox/AO_Projects/met_council_analyses/2_transit/scenarios/GEOID10_GNISID.csv"
        ctu_names = input("Type path to CTU id + CTU name strings file (default available): ") or "/Users/kristincarlson/Dropbox/AO_Projects/met_council_analyses/2_transit/scenarios/CTU_NAMES.csv"
    ...
    if args.COMPARE_FLAG == "Y":
        ...
        try:
            ...
            # check is folder has already been averaged
            arr = os.listdir('./')
            if "Access_Results_Avg" not in arr:
                # Begin operations
                access.unzipBZ2()
                bar.next()
                print("\n")
