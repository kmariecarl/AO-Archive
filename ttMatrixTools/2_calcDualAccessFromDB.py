# This program loads POI data to Postgres database then computes the dual accessibility (how long to reach 1, 2, 3, 10
# destinations of type X). Output format should be a .csv

# This program relies on the sorted order of the incoming data. The SQL query orders the data on origin and traveltime

# Notes, data will be joined on block ids (GEOID10) so tt_median table and poi table must be associated with a block id
...
# Removed on 6/16 to reduce number of operations
# def join_tables(tt_tab, poi_tab, tt_poi_tab, percentile, cursor):
#     print(f"\n Creating a joined table of origins, {percentile} percentile travel times, and POIs ")
#     query = f"""create table {tt_poi_tab.db}.{tt_poi_tab.schema}.{tt_poi_tab.table} as
#                 select A.origin, A.destination, A.{percentile}, B.* from {tt_tab.db}.{tt_tab.schema}.{tt_tab.table} A
#                 left join {poi_tab.db}.{poi_tab.schema}.{poi_tab.table} B on A.destination = B.geoid10;"""
#     cursor.execute(query)
#     print(cursor.mogrify(query))
#     print(datetime.now())
...
def make_view(tt_tab, poi_tab, poi_list, poi_column, percentile):
    ...
    for poi in poi_list:
        if poi_list.index(poi) == 0:
            summation_str = summation_str + f"{poi_tab.schema}.{poi_tab.table}.{poi}"
        summation_str = summation_str + f" + {poi_tab.schema}.{poi_tab.table}.{poi}"
    print(summation_str)
...
def calc_dual(poi_view, percentile, poi_column, o_list, limit, cursor):
    ...
    # Returns list of tuples [(...), (...),...] spot 0 is geoid10, spot 1 is tt, spot 2 is num of
...
if __name__ == '__main__':
    ...
    parser.add_argument('-tt_table', '--TT_DECILES_TABLE_NAME', required=True,
    ...
    parser.add_argument('-poi_num', '--SINGLE_OR_MULTIPLE', required=True,
    ...
    tt_tab = DBObject(args.DB_NAME, args.SCHEMA_NAME, args.TT_DECILES_TABLE_NAME)
    ...
    try:
        ...
        load_poi = input("If POI data table has not been loaded to db, type 'load', else type 'x': ")
        print("NOTE: The program assumes GEOID10 is the name of the block id column \n")

        # If the POI table has not loaded to db yet, do so now
        if load_poi == "load":
            column_str = "(GEOID10 BIGINT,"
            with open(args.POI_FILE_PATH, 'r') as f:
                poi_dict = csv.DictReader(f)
                print("POI file fieldnames \n", poi_dict.fieldnames)
                for i in poi_dict.fieldnames:
                    if i != "GEOID10":  #This column is dealt with earlier
        ...
        # if len(poi_list) > 1:
        view_name = make_view(tt_tab, poi_tab, poi_list, poi_column, percentile)
